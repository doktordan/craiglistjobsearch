...
var initialize = (function (self) {
	...
	*/
	...
	function getJobs (options, html) {
		var
			$ = cheerio.load(html),
			job = {},
			jobs = [];
			console.log();
		$('#jjj0').find('li').each(function (i, element) {
			job = {
	...
	*/
	function getRequestOptions (options, query) {
		...
		if (options.cities){
			...
			searchPath = '/search/'+options.type+'?sort=rel&query=';
			requestOptions.path = searchPath + encodeURIComponent(query);
	...
	*/
	self.search = function (options, query, callback) {
		...
		self.request.get(options, function (err, data) {
			...
			}else if (options.jobs===true){
			 	return callback(null, getJobs(options, data));
			}else{
